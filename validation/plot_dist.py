...
for rank, ind in enumerate(val_shorted):
    ...
    plot = sns.distplot(X_train[y_train == 0, ind], bins=20, ax=axes[0])
    plot = sns.distplot(X_train[y_train == 1, ind], bins=20, ax=axes[0])
    plot = sns.distplot(X_val[y_val == 0, ind], bins=20, ax=axes[1])
    plot = sns.distplot(X_val[y_val == 1, ind], bins=20, ax=axes[1])
    plt.savefig(str(rank) + "_" + df_train[markers].columns[ind] + ".pdf")
    plt.close(fig)
