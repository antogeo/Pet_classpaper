...
for t_iter, (train, test) in enumerate(sss.split(X, y)):
    for val in weight_val:
        classifiers['RF_w_15'] = Pipeline([
                ('scaler', RobustScaler()),
                ('clf', RandomForestClassifier(
                        max_depth=15, n_estimators=2000, max_features='auto',
                        class_weight={0: 1, 1: val}))
            ])
        classifiers['RF_w_10'] = Pipeline([
                ('scaler', RobustScaler()),
                ('clf', RandomForestClassifier(
                        max_depth=10, n_estimators=2000, max_features='auto',
                        class_weight={0: 1, 1: val}))
            ])
        classifiers['RF_w_5'] = Pipeline([
                ('scaler', RobustScaler()),
                ('clf', RandomForestClassifier(
